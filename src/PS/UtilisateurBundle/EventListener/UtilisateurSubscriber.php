...
class UtilisateurSubscriber implements EventSubscriberInterface
{
    ...
    public function preSetData(FormEvent $event)
    {
        ...
        if ($user->hasRole('ROLE_ADMIN') || $user->hasRole('ROLE_SUPER_ADMIN') || $user->hasRole('ROLE_ADMIN_SUP')) {
            $roles = array_merge($roles, ['ROLE_PHARMACIE' => 'Pharmacien',
                        'ROLE_RECEPTION' => 'Réceptionniste',
                        'ROLE_INFIRMIER' => 'Infirmier',
                        'ROLE_MEDECIN'   => 'Medecin',
            ...
            if ($user->hasRole('ROLE_ADMIN')) {
                ...
                //unset($roles['ROLE_ADMIN_LOCAL']);
            } elseif ($user->hasRole('ROLE_ADMIN_SUP')) {
                $roles = ['ROLE_ADMIN_LOCAL' => 'Admin Local', 'ROLE_PHARMACIE' => 'Pharmacien'];
                //unset($roles['ROLE_ADMIN_SUP'], $roles['ROLE_SUPER_ADMIN'], $roles['ROLE_ADMIN'], $roles['ROLE_MEDECIN'], $roles['ROLE_INFIRMIER']);
            }
        ...
        if ($user->hasRoles(['ROLE_SUPER_ADMIN', 'ROLE_ADMIN'])) {
            $form->add('assurance', EntityType::class
                , [
